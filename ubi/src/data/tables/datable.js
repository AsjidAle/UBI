...
import CompaniesPopup from "../../components/Companies/CompaniesPopup";
import { toast } from 'react-toastify';
import Swal from "sweetalert2";
import Utils from "../../utils/Utils";

export function Companiesdatatable({ companies, setCompanies }) {
  ...
  const columns = [
    ...
    },
    {
      ...
      selector: row => {
        if (row.logo)
          return <span className="main-img-user mx-1"><img className="wd-35 ht-35" src={process.env.REACT_APP_UPLOADS_PUBLIC_URL + 'companies/' + row.logo} alt={row.name} /></span>;
        else
          return <></>;
      },
      sortable: false,
    ...
    {
      name: "CLIENTS",
      selector: row => [row.user_ids.length],
      sortable: true
    },
    {
      name: "ACTION",
  ...
  return (
    <>
      ...
      <CompaniesPopup showModal={showModal} setShowModal={setShowModal} company={company} companies={companies} setCompanies={setCompanies} />
    </>
  );
}
